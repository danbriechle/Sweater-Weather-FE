...
$.get(weather_url, function(data,status){
  ...
  $("#feels").text([(`${data["current"]["feels"]}°`).substring(0,2),"°"].join(''))
  $("#humidity").text([(`${data["current"]["humdity"]}`).substring(2, 4),"%"].join(''))
...
$("#change-location").click(function() {
  currentLocation = $("#city-state-input").val()
  imageCall(currentLocation);
  weatherCall(currentLocation);
});

$("#hourly-forecast").click(function() {
  $(".hourly-forecast").toggle("slide");
});

$("#five-day-forecast").click(function() {
  $(".five-day-list").toggle("slide");
});

// this syntax makes the clock conststanly increment (still learning why)
jQuery(function($) {
// sets the interval function (basically incrementing the the time portion of the string)
  setInterval(function() {
// returns the time portion of the date as a string (Im thinking this uses the ip address as I can't get it to update
// based on the seleceted area )
